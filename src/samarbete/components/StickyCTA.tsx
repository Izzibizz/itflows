...
export const StickyCTA: React.FC = () => {
 ...
 useEffect(() => {
  ...
  console.log(state.succeeded, setHideSticky);
  return (
    <section
      className={`animate-fadeIn ${hideSticky && "hidden"} ${
        isPopup && !popupHasBeenSeen
          ? "w-screen h-screen bg-warm-black/70 text-warm-black z-80 "
      ...
      {isPopup && !popupHasBeenSeen ? (
        ...
        >
          <div className="flex flex-col tablet:flex-row justify-between relative ">
            ...
            />
            {state.succeeded ? (
              <div className="animate-fadeIn flex flex-col gap-2 h-full p-6 tablet:w-2/3 items-center text-center justify-center  min-h-[500px] ">
              ...
              >
                <div className="flex flex-col gap-2 items-center self-center">
                  <h3 className="text-xl tablet:text-2xl">
                    ...
                    <span className="text-2xl tablet:text-3xl laptop:text-4xl font-collab">
                      första konsultation
                    </span>
                  </h3>
                  <h4 className="text-xl tablet:text-2xl self-end">
                    helt utan bindning
                  </h4>
                </div>
                <div className="flex flex-col gap-2">
                  <label className="flex gap-2">
                    <span className="whitespace-nowrap">Namn:</span>
                    <input
                      type="text"
                      name="name"
                  ...
                  />
                  <label className="flex gap-2">
                    <span className="whitespace-nowrap">Företag:</span>
                    <input
                      type="text"
                      name="företag"
                      className="border border-dotted border-collab-green rounded-lg w-full"
                  ...
                  />
                  <label className="flex gap-2">
                    <span className="whitespace-nowrap">E-post:</span>
                    <input
                      id="email"
                      type="email"
                      name="email"
            ...
            <video
              ...
              loop
              playsInline
              className="hidden tablet:block tablet:w-1/2 tablet:aspect-[3/4] laptop:h-full object-cover object-[center_35%] tablet:object-center rounded-b-4xl tablet:rounded-r-4xl tablet:rounded-l-none"
            />
          </div>
        </div>
      ) : (
